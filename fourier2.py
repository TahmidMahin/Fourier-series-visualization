...
pg.display.set_caption("Fourier Visualizations 2")
...
while running:
	...
	centers = [[round(starts[0][0]), round(starts[0][1])]]

	for k in range(1, wave_number):
		starts.append(ends[-1])
		radii.append(radii[0]/(k+1))
		if k%2 == 1:
			ends.append([starts[-1][0] + radii[-1]*math.cos((k+1)*t), starts[-1][1] + radii[-1]*math.sin((k+1)*t)])
		else:
			ends.append([starts[-1][0] + radii[-1]*math.cos((k+1)*t), starts[-1][1] - radii[-1]*math.sin((k+1)*t)])
		centers.append([round(starts[-1][0]), round(starts[-1][1])])

	for k in range(0, wave_number):
		pg.draw.line(screen, white, starts[k], ends[k], 1)
		pg.draw.circle(screen, white, centers[k], round(radii[k]), 1)
